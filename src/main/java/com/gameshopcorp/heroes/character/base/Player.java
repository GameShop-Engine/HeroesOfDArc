    ...
    public void makeATMS(){
        ...
        for (Base d: parts){
            ...
            if (d != null) {
                ...
                h = d.superMesh.atms.height * parts.size();
                break;
            }
        }
...
//        }
        ...
        for (Base sm: parts){
            if (sm != null){
                totalVertices += sm.superMesh.vertices.length;
            }
        }
        ...
        for (Base sm: parts){
            if (sm != null){
                totalIndices += sm.superMesh.texCoord.length;
            }
        }
        ...
        for (Base sm: parts){
            if (sm != null){
                totalIndexes += sm.superMesh.indexes.length;
            }
        }
        ...
        for (Base sm : parts){
            ...
            if (sm != null) {
                for (int x = 0; x < sm.superMesh.vertices.length; x++) {
                    vertices[x + (v)] = new Vector3f();
                    vertices[x + (v)] = sm.superMesh.vertices[x];
                    //System.out.println("V " + v);
                }
                v += sm.superMesh.vertices.length;

                for (int x = 0; x < sm.superMesh.texCoord.length; x++) {

                    texCoord[x + (t)] = new Vector2f();//sm.texCoord[x].mult(it, 1);
                    Vector2f finalTexCoord = sm.superMesh.texCoord[x].add(new Vector2f(0, it));
                    texCoord[x + (t)] = new Vector2f(finalTexCoord.x, finalTexCoord.y/parts.size());
                }

                t += sm.superMesh.texCoord.length;

                for (int x = 0; x < sm.superMesh.indexes.length; x++){

                    indexes[x + (i)] = sm.superMesh.indexes[x] + (y);// + (i);


                }
                y += sm.superMesh.vertices.length;

                //indexes[i + 6] = (short) -1;

                i += (sm.superMesh.indexes.length);
